...
- **Python version f-string incompatibility.** Nested f-strings with quoted keys (e.g. `f"…{"key" if cond else "val"}…"` are valid in Python 3.12+ but raise `SyntaxError` in 3.11. If a spike fails with a syntax error on a valid-looking f-string, extract the problematic expression to a variable first:
  ```python
  # Python 3.11 FAILS:
  return f"央行{direction}政策，流动性{"改善" if s > 0 else "收紧"}"
  # Python 3.11 OK:
  val = "改善" if s > 0 else "收紧"
  return f"央行{direction}政策，流动性{val}"
  ```
- **Missing venv / externally-managed environment.** On some systems (Debian/Ubuntu), `pip install` fails with `externally-managed-environment`. Use `uv venv .venv && uv pip install <pkg> -p .venv` instead. Activate with `source .venv/bin/activate`.
- **Missing packages found at runtime.** If a spike exposes missing imports, install them all at once rather than one-by-one (each missing dep usually reveals another). Collect all imports first:
  ```python
  import re, os
  packages = set()
      ...
      for f in files:
          if f.endswith(".py"):
              for line in open(os.path.join(root, f)):
                  m = re.match(r'^\s*(?:from|import)\s+(\w+)', line.strip())
                  if m: packages.add(m.group(1))
  stdlib = {"sys","os","re","datetime","pathlib",...}
  print(sorted(packages - stdlib - {"my_package"}))
  ```
  Then install all at once: `uv pip install <pkg1> <pkg2> ... -p .venv`
